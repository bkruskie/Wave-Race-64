...
/**
 * Saves time trial records for a slot.
 * Encodes 3 records from D_801C26E8 using func_8007B1AC into D_801AEA18->unk120,
 * then writes header + data to EEPROM.
 */
s32 func_8007C0B8(s32 slot) {
    s32 i;
    s32 temp_v0;
    ...
    for (i = 0; i < 3; i++) {
        func_8007B1AC((Unkstruct_8007B1AC_arg0*) &D_801C26E8[slot][i],
                      (Unkstruct_8007B1AC_arg1*) D_801AEA18->unk120[slot][i]);
    }
    ...
    temp_v0 = D_800D826C[slot];
    if (osEepromLongWrite(&D_801540D0,
                          (((uintptr_t) &D_801AEB38 - (uintptr_t) &D_801AEA18) + temp_v0) >> 3,
                          (u8*) D_801AEA18->unk120 + temp_v0,
                          0x18) != 0) {
        return 3;
    }
    return 0;
...
/**
 * Saves game data to EEPROM.
 * Returns 1 if no EEPROM present, 3 on write error, 0 on success.
 * Copies course/weather data via func_8007B31C, generates checksum,
 * then writes the 16-byte save block to EEPROM.
 */
